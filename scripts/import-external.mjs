...
async function copyItem({ from, to, overwrite }, sourceRoot, opts) {
  const fromAbs = path.isAbsolute(from) ? from : path.join(repoRoot, sourceRoot, from);
  const toAbs = path.isAbsolute(to) ? to : path.join(repoRoot, to);

  const effectiveOverwrite = typeof overwrite === "boolean" ? overwrite : !!opts.overwrite;

  if (!(await exists(fromAbs))) {
    console.warn(`Skipping: source not found -> ${fromAbs}`);
    return { skipped: true, reason: "missing-source", from: fromAbs, to: toAbs };
  }

  await ensureDir(path.dirname(toAbs));

  if (opts.dry) {
    console.log(`[DRY] Copy ${fromAbs} -> ${toAbs} overwrite=${effectiveOverwrite}`);
  ...
  console.log(`✓ Copied ${fromAbs} -> ${toAbs}${effectiveOverwrite ? " (overwrote existing)" : ""}`);
...
async function main() {
  ...
  if (!manifest.items.length) {
    console.log("No items in manifest to import. Edit external_imports/manifest.json.");
    console.log("Example item: { \"from\": \"components/ui\", \"to\": \"components/ui\", \"overwrite\": false }");
    return;
  }

  console.log(`Importing ${manifest.items.length} item(s) from ${sourceRootAbs}`);
